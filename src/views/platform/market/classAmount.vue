<template>
    <div class="taskAmountCon">
        <el-form :model="formDetail" label-width="120px" label-position="left" class="formSearchMarginTop">
            <el-form-item label="班务抢单状态：" class="formSearchMargin">
                <el-radio-group class="radio-search" v-model="formDetail.receiveStatus">
                    <el-radio label="">不限</el-radio>
                    <!--1.待确认,2.领取失败,3.领取成功,4.已放弃）-->
                    <el-radio label="1">待确认</el-radio>
                    <el-radio label="2">领取失败</el-radio>
                    <el-radio label="3">领取成功</el-radio>
                    <el-radio label="4">已放弃</el-radio>
                </el-radio-group>
            </el-form-item>
        ...
        <el-table class="table-outline" :data="tableData" empty-text="暂无相关信息">
            ...
            <el-table-column prop="classTaskName" label="班务名称" min-width="100"></el-table-column>
            <el-table-column prop="assignmentInfoName" label="任务名称" min-width="100"></el-table-column>
        ...
        <!--查看记录-->
        <el-dialog :title="`${viewInfo.classTaskName} - 保证金交易记录`" :visible.sync="dialogInsuVisible" width="1100px">
        ...
        methods: {
            ...
            async getData() {
                const params = {
                    ...
                    receiveStatus: this.formDetail.receiveStatus,
                    depositStatus: this.formDetail.depositStatus,//（1.已支付 2.退回失败  3.已退回）"
                };
                const {data} = await this.$api.ctList(params);
                if(data.respCode === 0){
                    if(data.data.list){
